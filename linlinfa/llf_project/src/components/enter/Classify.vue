	...
	<div class="classify">
		...
		<div class="classify_level">
			<van-row>
					...
					<vue-better-scroll ref="scroll1" :startY="parseInt(startY)">
						<ul class="classify_level1_ul">
							<li :class="{active_level1:index== current1}" v-for="(listArr,index) in list" 
							 :key="listArr.gc_id" @touchstart="clickLevel1(index,listArr)">
							 <span>
								 {{listArr.gc_name}}
							 </span>
							</li>
				...
				<van-col :span="8">
					<vue-better-scroll ref="scroll2" :startY="parseInt(startY)"  v-if="list[0].children[0].children[0]" >
						<ul  class="classify_level2_ul">
								<li :class="{active_level2:index2 == current2}" v-for="(listArr2,index2) in list[current1].children" @touchstart="clickLevel2(index2,listArr2)" :key="listArr2.gc_id">
									{{listArr2.gc_name}}
								</li>
								<li></li>
						</ul>
				...
				<van-col :span="9">
					<vue-better-scroll ref="scroll3" :startY="parseInt(startY)"  v-if="list[0].children[0].children[0]" >
					<ul class="classify_level3_ul">
						<li :class="{active_level3:index3 == current3}" v-for="(listArr3,index3) in list[current1].children[current2].children" @touchstart="clickLevel3(index3,listArr3)" :key="listArr3.gc_id">{{listArr3.gc_name}}</li>
						<li></li>
					</ul>
				</vue-better-scroll>
				</van-col>
...
<script>
	...
	export default {
      ...
      //post value
			...
			this.$axios.post("/v3/gc_list",formData,{headers: {"Content-Type":"application/json"}})
      .then((data)=>{
				// /v3/area_list
